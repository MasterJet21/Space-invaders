# Player requirement:
# 1. Image of the player
# 2. Move the player
# 3. Constrain the player to the WindowsError
# 4. Able to shoot lasers and recharge

import pygame, sys
# from Games.Space_invaders.Space_invaders_player import Player
from Space_invaders_player import Player
import obstacle
...
# Lasers(Conditions):
# Player lasers -> Obstacles and Aliens
# Aliens -> Obstacles and Player
    ...
    def create_obstacle(self, x_start, y_start, offset_x):
        for row_index, row in enumerate(self.shape):
            for col_index, col in enumerate(row):
                if col == 'x':
                    x = x_start + col_index * self.block_size + offset_x # x_start(distance from the left)
                    y = y_start + row_index * self.block_size 
                    block = obstacle.block(self.block_size, (241, 79, 80), x, y)
                    self.blocks.add(block)

    def create_multiple_obstacles(self, *offset, x_start, y_start): # *offset is an unpacking operator.
        ...
        if self.alien_lasers:
            for laser in self.alien_lasers:
                ...
                if pygame.sprite.spritecollide(laser, self.player, False):
                    music = pygame.mixer.Sound('explosion.wav')
